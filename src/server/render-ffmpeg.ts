...
// ─── Ken Burns variants (matches preview-player.tsx order) ──────────────────
// Seeded by mediaId so preview and render always use the same variant.
...
function kenBurnsFilter(mediaId: string, duration: number, w: number, h: number): string {
  ...
  // zoom delta per frame so total zoom change = 0.15 over d frames
  const dz  = (0.15 / d).toFixed(6);
  // pan delta per frame in source pixels (source is 2× output, pan up to 15% of output)
  const pan = Math.round(w * 0.15 * 2 / d * 100) / 100;

  const exprs: string[] = [
    // 0: zoom-in center
    `z='min(zoom+${dz},1.3)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)'`,
    // 1: drift right (no zoom change)
...
import { getJob, saveJob, saveJobOutputPath } from "@/server/job-store";
...
async function runJob(jobId: string, input: RenderRequest, summary: CompositionSummary) {
  ...
  try {
    update("montando_video", 15, "Iniciando FFmpeg...");

    const serverPaths: Record<string, string> = {};
    for (const m of input.media) {
      if (m.serverPath) serverPaths[m.id] = m.serverPath;
    }
    ...
    const outputPath = await renderVideo(
      ...
      input.preset,
      jobId,
      (pct) => update("montando_video", 15 + Math.round(pct * 0.77), `Renderizando... ${pct}%`),
      input.textOverlays,
      input.watermarks,
      input.outputOptions ?? DEFAULT_OUTPUT_OPTIONS,
    );

    saveJobOutputPath(jobId, outputPath);
...
async function renderVideo(
  ...
  const outputDir = join(tmpdir(), "qlipo", "renders", jobId);
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    // Sort segments by startAt for correct ordering
    const sortedSegs = [...summary.visualSegments].sort((a, b) => a.startAt - b.startAt);
    const withGaps = detectGaps(sortedSegs);

    // ── Visual inputs & filters ─────────────────────────────────────────────
    const videoLabels: string[] = [];
    let fileInputIdx = 0;
    let blackIdx = 0;

    // applyFadeIn / applyFadeOut let callers suppress individual fades
    // when xfade handles the transition (avoids double-fade artifacts).
    const processClip = (
      seg: VisualSegment,
      applyFadeIn = true,
      applyFadeOut = true,
      ...
      const path = serverPaths[seg.mediaId];
      if (!item || !path) return;
      ...
      if (isImage) {
        ...
        const srcTrimEnd = Number((duration * speed).toFixed(3)); // source frames needed
      ...
      if (applyFadeIn && seg.fadeInSeconds > 0) {
        f += `,fade=t=in:st=0:d=${seg.fadeInSeconds}`;
      }
      ...
      if (brightness !== 1 || contrast !== 1) {
        f += `,eq=brightness=${brightness - 1}:contrast=${contrast}`;
      }
    ...
    if (withGaps) {
      // ── Manual mode with gaps: fill gaps with black, concat everything ─────
      let prevEnd = 0;
      ...
      // Concat all labels (clips + gap blacks)
      if (videoLabels.length === 0) {
        reject(new Error("Nenhum segmento visual disponivel para renderizar.")); return;
      }
      ...
      // ── No gaps: use xfade chain or simple concat ─────────────────────────
      const hasXfade = sortedSegs.some(
        (s) => s.transitionType && s.transitionType !== "fade",
      );

      if (!hasXfade) {
        // Simple path: individual fades + concat
        ...
        // xfade path: suppress per-clip fade filters on edges handled by xfade,
        // then build a chain [v0][v1]xfade[xf0]; [xf0][v2]xfade[xf1]; ...
        const n = sortedSegs.length;
        for (let si = 0; si < n; si++) {
          const isFirst = si === 0;
          const isLast  = si === n - 1;
          // Keep fade-in only on very first clip; keep fade-out only on very last clip.
          // Middle clips: xfade handles the transition — no individual fades.
          processClip(sortedSegs[si], isFirst, isLast);
        }
        ...
        if (videoLabels.length === 1) {
          ...
          for (let si = 1; si < n; si++) {
            ...
            const xType    = (seg.transitionType && seg.transitionType !== "fade")
            ...
            filters.push(
              `${currentLabel}${incoming}xfade=transition=${xType}:duration=${xDur}:offset=${offset}${outLabel}`,
            );

            currentLabel  = outLabel;
            const segDur  = Number((seg.endAt - seg.startAt).toFixed(3));
            cumulativeDur = Number((offset + segDur).toFixed(3));
          }
        }
      }
    }
    ...
    if (textOverlays && textOverlays.length > 0) {
      // Rename the final [vout] to [vtbase] so we can chain drawtexts
    ...
    // ── Watermarks (overlay with scale & fade) ──────────────────────────────
    if (watermarks && watermarks.length > 0) {
      // Rename the final [vout] to [vw_base] so we can chain overlays
      const lastIdx = filters.length - 1;
      if (filters[lastIdx]?.endsWith("[vout]")) {
      ...
      for (let wmIdx = 0; wmIdx < watermarks.length; wmIdx++) {
        ...
        const wmPath = serverPaths[wm.mediaId];
        ...
        // Build scale filter for watermark
        filters.push(`[${wmFileIdx}:v]scale=${wmWidth}:-1,format=rgba[wm${wmIdx}]`);

        // Build overlay with fade opacity expression
        ...
        const alphaVal = Math.round((wm.opacity / 100) * 255);

        const outLabel = wmIdx === watermarks.length - 1 ? "[vout]" : `[vw${wmIdx}]`;
        filters.push(
          `${prevLabel}[wm${wmIdx}]overlay=x=${Math.round(x)}:y=${Math.round(y)}:enable=’${enable}’:alpha=’${opacityExpr}’${outLabel}`
        );
    ...
    for (let i = 0; i < summary.audioSegments.length; i++) {
      ...
      if (seg.fadeInSeconds > 0) {
        f += `,afade=t=in:st=0:d=${seg.fadeInSeconds}`;
      }
    ...
    // ── Output options (codec / quality / fps / audio) ──────────────────────
    ...
    if (audioLabels.length > 0) {
      ...
      if (audioLabels.length === 1) {
        ...
        filters.push(
          `${audioLabels.join("")}amix=inputs=${audioLabels.length}:duration=first:normalize=0,aresample=${sampleRate}[aout]`,
        );
      }
      const audioCodec = isVp9 ? "libopus" : "aac";
      ffmpegOutputOptions.push("-map", "[aout]", `-c:a`, audioCodec, `-b:a`, `${audioBitrate}k`);
    }

    // Rename local var to avoid collision with outer outputOptions
